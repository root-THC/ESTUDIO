...
`COALESCE(comm, 0)` → Si `comm` es `NULL`, se reemplaza por `0` \
 Se usa para poder operarlo, ya que no es lo mismo hacer la media por ejemplo de todos los valores a que haya `NULLS` los ignorara y el resultado será un valor incorrecto a que sea `O`
