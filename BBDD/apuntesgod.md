...
`COALESCE(comm, 0)` → Si `comm` es `NULL`, se reemplaza por `0` \
 Se usa para poder operarlo ya que no es lo mismo hacer la media por ejemplo de todos los valores ya que los `NULL` no los cojera a que sea 0 que es lo mismo en caso de que tenga valor pues no hace nada lo mue
